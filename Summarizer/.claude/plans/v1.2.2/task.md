...
### [완료] BugFix-3. `PhoneNumberRegex` — '/' 및 한글 텍스트 소비 문제 수정

- **증상**: "김철수/010-3000-5000" → `[ 010-3000-5000 ]` (정상: `[ 김철수/010-3000-5000 ]`)
  - 이름 및 구분자 '/'가 전화번호 앞에 있을 경우 잘려나감
- **원인**: 기존 `PhoneNumberRegex`의 `.*?` 패턴이 비탐욕적(non-greedy)이지만 첫 매칭 위치를 최대한 앞으로 당기기 때문에, "김철수/"까지 소비하여 매치 영역에 포함시킴. 결과적으로 `match.Index`가 "김"의 위치가 되고, `builder.Append(phoneNumberText, 0, match.Index)`가 빈 문자열(또는 앞 일부)만 출력함
- **수정**: `PhoneNumberRegex`를 alternation 구조로 변경
  - 변경 전: `\+?\b8?2?.*?0?10.*?\d{4}.*?\d{4}\b`
  - 변경 후: `(?:\+?\b8?2[-\s.]*|\b)0?10[-\s.]*\d{4}[-\s.]*\d{4}\b`
  - 국제번호 경로 `\+?\b8?2[-\s.]*`: 82 국가코드 시작 시만 구분자(`[-\s.]`) 허용
  - 로컬번호 경로 `\b`: 010 직전의 단어 경계만 허용 — 한글/영문/슬래시 앞 위치를 소비하지 않음
- **수정 파일**: `Summarizer.Core/MessageConverter.cs`
