...
- 실제 대화 내용이나 개인정보가 담긴 파일은 **절대로 저장소에 커밋해서는 안 된다.**
  - 저장소 등록 대상인 파일 중에서 대화 내용, 이름, 연락처 등 개인정보가 포함된 것으로 판단되는 텍스트 및 데이터를 발견한 경우, 이를 저장소에 커밋할지 여부를 승인받아야 한다.
  - 저장소 등록으로 승인된 항목들(테스트를 위한 가상 인물 및 시나리오 등)은, 이를 쉽게 식별할 수 있는 말머리를 달아서 구분할 수 있다. (`[가상]` 또는 `[테스트]` 등)
- 기능 테스트를 위한 예제 시나리오가 필요한 경우, **반드시 허구의 인물과 가공의 내용**으로 작성한다.
  - 실제 이름, 실제 전화번호, 실제 진료 항목과 날짜의 조합 등, 어떠한 개인 및 개인의 특정한 활동 정보를 식별 가능할 수 있는 정보는 프로젝트 저장소에 포함해서는 안 된다.
